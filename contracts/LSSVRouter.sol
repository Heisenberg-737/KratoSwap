...
contract LSSVMRouter {
    ...
    //give eth and get back any nfts. uses an internal function for the swap
    ...
    //give nfts and receive specified NFTs. also uses internal function
    ...
    //swaps one set of nfts into another set of nfts. Uses Eth for an intermediary step
    // e.y nfts -> eth -> nft
    ...
    //swaps a set of nfts into another set of specified nfts. Uses Eth for an intermediary step
    // e.y nfts -> eth -> nft
    ...
    //swap an ERC20 token to receive any nft
    ...
    //swap an erc20 token to recieve specified nfts
    ...
    //swap one set of nfts into another set, using an erc20 as an intermediary
    //i.e nft -> erc20 token -> nft
    ...
    //swap one set of nfts to another set of specific nfts using an erc20 token as an intermediary
    //i.e nft -> erc20 token -> nfts
    ...
    /**
        ...
        Instead, users specify a per-swap max cost. If the price changes more than the user specifies, 
        no swap is attempted. This allows users to specify a batch of swaps, and execute as many of them as possible.

        To prevent revertion, we assume the value sent(msg.value) is greater than the maxCostPerPair 
     */

    //swap as much eth as possible for nfts and sent remaining balance to ethReceipient
    ...
    //swap as much eth as possible for specified nfts and sent remaining balance to ethReceipient
    ...
    //swap as much as possible erc20 tokens for nfts
    ...
    //swap as much as possible erc20 tokens to specified nfts
    ...
    //swap as many nfts for tokens as possible
    ...
    //buy nfts with eth and sell them for tokens in one transaction
    ...
    // Buys NFTs with erc20 tokens and sell them for tokens in one transaction
    ...
    // Allows an ERC20 pair contract to transfer ERC20 tokens directly from
    ...
    //Allows a pair contract to transfer ERC721 NFTs directly from
    ...
    //checks deadline valid for a swap to happen
    ...
    //swap an erc20 token for specific nfts
    ...
    //swap nfts for tokens, to be used for 1 token at a time
